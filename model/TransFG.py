...
def np2th(weights, conv=False):
    ...
    if conv:
        weights = weights.transp7ose([3, 2, 0, 1])
...
class Attention(nn.Module):
    def __init__(self, config):
        ...
        self.attention_head_size = int(config.hidden_size / self.num_attention_heads)  #multi-head ,to make x cut to the piece
    ...
    def forward(self, hidden_states):
        ...
        """print("this is shape of hidden_states:")
        ...
        """print("this is shape of q,k,v")
        print(mixed_query_layer.shape,mixed_key_layer.shape,mixed_value_layer.shape)"""

        query_layer = self.transpose_for_scores(mixed_query_layer)  #q1
        key_layer = self.transpose_for_scores(mixed_key_layer)      #k1
        value_layer = self.transpose_for_scores(mixed_value_layer)  #v1
        
        """print("this is shape of q_layer,k_layer,v_layer:")
        print(query_layer.shape,key_layer.shape,value_layer.shape)"""

        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))  #q and k xiang cheng
        """print("this is shape of q*k")
        print(attention_scores.shape)"""
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)  #a1,1
        attention_probs = self.softmax(attention_scores)
        weights = attention_probs
        attention_probs = self.attn_dropout(attention_probs)

        context_layer = torch.matmul(attention_probs, value_layer)   #b1 
        """print("this is shape of context layer:")
        print(context_layer.shape)"""
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
        """print("this is shape of new_context_layer.shape:")
...
class Embeddings(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(x.shape)    
        #[12,65,768]
        #print(self.position_embeddings.shape)  [1,65,768]
...
class Part_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(1, length):
            ...
            """print("this is matmul last map shape")
            print(last_map.shape)"""
        last_map = last_map[:,:,0,1:]
        """print("this is last_map afer")
        print(last_map.shape)"""

        _, max_inx = last_map.max(2)
        """print("this is last_map after max")
        print(last_map.shape)"""
        return _, max_inx
...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states): 
        ...
        for layer in self.layer:    
            ...
            """print("this is weights shape in layer:")
            print(weights.shape)"""
            attn_weights.append(weights)
        
        """print("this is hidden_states shape:")
        print(hidden_states.shape)
        
        print("this is weigths shape:")
        print(weights.shape)
        ...
        """print("this is part num:")
        print(part_num)"""
        
        part_inx = part_inx + 1
        """print("this is part inx")
        print(part_inx)
        print("this is part_num,part_inx shape")
        print(part_num.shape,part_inx.shape)"""
        parts = []
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=128, num_classes=3, smoothing_value=0, zero_head=False):
    ...
    def forward(self, x, labels=None):
        ...
        part_logits = self.part_head(part_tokens[:, 0])  #clasiification head
