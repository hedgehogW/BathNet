...
class RelPosSelfAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        """print("this is logits shape:")
        print(logits.shape)"""
        weights = torch.reshape(logits, [-1, heads, h, w, h * w])
        """print("this is original weights:")
        print(weights.shape)"""
...
class MHSA(nn.Module):
    ...
    def forward(self, input):
        ...
        o , weights  = self.self_attention(q=q, k=k, v=v)
        return o , weights
...
class GTBotBlock(nn.Module):
    ...
    def __init__(self, in_dimension, curr_h, curr_w, proj_factor=4, activation='relu', pos_enc_type='relative',
        ...
        self.mhsa2 = MHSA(in_channels=bottleneck_dimension, heads=4, curr_h=curr_h*2, curr_w=curr_w*2,
                         pos_enc_type=pos_enc_type)

        self.mhsa3 = MHSA(in_channels=bottleneck_dimension, heads=4, curr_h=curr_h*4, curr_w=curr_w*4,
                         pos_enc_type=pos_enc_type)
        ...
        #GTBotblock2 and botblock
        self.conv4 = nn.Sequential(
            nn.Conv2d(target_dimension, bottleneck_dimension, kernel_size=1, stride=1),
            BNReLU(bottleneck_dimension, activation=activation, nonlinearity=True)
        )

        stride2 =2 if 1 == 0 else 1
        conv5_list = []
        conv5_list.append(BNReLU(bottleneck_dimension, activation=activation, nonlinearity=True))
    ...
    def forward(self, x):
        ...
        out , weights = self.mhsa(out)
        """print("this is weights shape")
        print(weights.shape)"""
        ...
        out , weights = self.mhsa2(out)
        weights = torch.reshape(weights,[4, N,-1, Q_h , Q_w])
        attn_weights.append(weights)
        #print("this is mhsa2 weights shape")
        #print(weights.shape)
        out = out.permute(0, 3, 1, 2)  # back to pytorch dim order
        out = self.conv5(out)
        N1, C1, H1, W1 = out.shape
        ...
        part_num, part_inx = self.part_select(weights) #all shape is [4,8,8]
        part_inx = part_inx + 1
        """print(part_inx)
        print(part_inx.shape)"""
        parts = []  
        #print(part_num.shape,part_inx.shape) 
        #N = part_inx.shape
        #B,num = part_inx.shape
        for i in range(N):
            parts.append(out[i, part_inx[i, :]])
            """print("parts shape:")
            print(parts[i].shape)"""
        parts = torch.stack(parts).squeeze(1)
        """print(parts.shape) #
        print(out.shape)   #12,512,16,16 """
        concat = torch.cat((out[:, 0].unsqueeze(1), parts), dim=1)
        #print(concat.shape)

        out, weigths = self.mhsa3(concat)
        """print(out.shape)
        print(weights.shape)"""
