...
class SiLU(nn.Module):
    def __init__(self, inplace=True):
        ...
        self.inplace = inplace   #inplace is True or Flase ,means whether to rewrite the previous value with the calculated value 

    def forward(self, x):
        #return SiLU(x, self.inplace)  
        return SiLUX(x, self.inplace)
...
#ConvBNAct and ConvBNSiLu's diffirence is act use Relu6 but bnsilu use SiLU
...
class ReXNetV1(nn.Module):
    def __init__(self, input_ch=3, final_ch=4, width_mult=1.0, depth_mult=1.0, classes=6,
                 use_se=True,
                 se_ratio=12,
                 dropout_ratio=0.2,
                 bn_momentum=0.9):
        ...
        print(layers)
        print(strides) #output is [1,2,1,2,1,2,1,1,1,1,1,2,1,1,1,1]
